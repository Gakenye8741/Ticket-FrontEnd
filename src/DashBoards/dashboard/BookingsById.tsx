...
const BookingsByNationalId: React.FC = () => {
  ...
  /**
   * 🔄 AUTO-REFRESH ENGINE
   * Refreshes the data every 10 seconds while the component is mounted.
   */
  useEffect(() => {
  ...
  const handleSecureCheckout = async (booking: BookingData) => {
    ...
    if (method === "mpesa") {
      ...
      watchdogRef.current = setInterval(async () => {
        ...
        if (verified) {
          stopWatchdog();
          // Sanitized update to avoid 500 error
          await updateBooking({ 
            bookingId: booking.bookingId, 
            body: { 
                bookingStatus: "Confirmed",
    ...
    <div className="min-h-screen bg-base-100 text-base-content p-4 md:p-12 mt-16 relative">
      ...
      <div className="max-w-7xl mx-auto space-y-10 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-base-300 pb-10">
          <div className="space-y-2">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-primary uppercase tracking-[0.5em]">Active Node // V.8.4</span>
             </div>
             <h1 className="text-5xl font-black tracking-tighter italic uppercase">
                {user?.firstName || "Member"}_<span className="text-primary opacity-80">VAULT</span>
             </h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
           {[
             { label: "Daily_Yield", value: analytics.today, icon: <Zap size={18}/>, color: "text-yellow-500" },
             { label: "Weekly_Sync", value: analytics.week, icon: <TrendingUp size={18}/>, color: "text-blue-500" },
             { label: "Monthly_Cap", value: analytics.month, icon: <Calendar size={18}/>, color: "text-purple-500" },
             { label: "Total_Settled", value: analytics.total, icon: <ShieldCheck size={18}/>, color: "text-success" }
           ].map((stat, i) => (
                  ...
                  <table className="table w-full border-separate border-spacing-y-4">
                    <thead>
                      <tr className="text-[10px] font-black uppercase text-base-content/40 tracking-[0.4em] border-none">
                        <th className="bg-transparent pl-8">Index</th>
                        <th className="bg-transparent">Event_Protocol</th>
                        <th className="bg-transparent text-center">Tier</th>
                        <th className="bg-transparent text-right">Units</th>
                        <th className="bg-transparent text-right">Settlement</th>
                        <th className="bg-transparent text-center">Status</th>
                        <th className="bg-transparent text-right pr-8">Command</th>
                      </tr>
                    </thead>
                    <tbody>
                      {paged.map((b) => {
        ...
        {/* FOOTER */}
        <div className="flex flex-col md:flex-row justify-between items-center px-10 gap-6 opacity-40">
           <div className="flex items-center gap-2"><div className="w-1.5 h-1.5 bg-success rounded-full animate-ping"></div><span className="text-[8px] font-black text-base-content uppercase tracking-[0.5em]">Realtime Sync</span></div>
           <div className="text-[8px] font-black text-base-content uppercase tracking-[0.8em] animate-pulse">Protocol_3.4.0_Encrypted</div>
           <div className="font-mono text-[8px] text-base-content uppercase tracking-widest">© 2026_GATEWAY_NET</div>
        </div>
      </div>
    </div>
  );
