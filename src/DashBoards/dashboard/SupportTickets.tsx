...
const UserSupportTickets = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { isConfirmed } = await SecurityModal.fire({
      ...
      text: isEdit ? 'This will modify the existing ticket entry.' : 'This will broadcast your request to the support node.',
      icon: 'question',
    ...
    try {
      if (isEdit) {
        ...
        toast.success('PROTOCOL_UPDATE: Success');
      } else {
        await createTicket(payload).unwrap();
        toast.success('NEW_TICKET_DEPLOAYED: Success');
      }
      resetForm();
      refetch();
    } catch {
  ...
  const handleDelete = async (ticketId: number) => {
    const result = await SecurityModal.fire({
      title: 'DELETE_TICKET?',
      text: 'This operation will permanently erase the support request from the central node.',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'CONFIRM_ERASE',
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-12 mt-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-6 border-b border-base-content/5 pb-10">
        <div className="flex items-center gap-4 text-center sm:text-left">
          ...
          <div>
            <h1 className="text-3xl md:text-4xl font-black italic uppercase tracking-tighter italic">Support_<span className="text-primary md:text-5xl">Hub</span></h1>
            <p className="text-[10px] font-mono opacity-50 uppercase tracking-[0.4em]">Node_Status: Active_Comm_Link</p>
          </div>
        </div>
        <button 
        ...
        >
          <Plus size={20} className="mr-2"/> New_Ticket
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 bg-base-200/30 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-base-content/5">
        ...
        <div className="relative group">
          ...
          >
            <option value="All">All_Tickets</option>
      ...
      {isLoading ? (
        <div className="flex flex-col justify-center items-center py-24 gap-4 opacity-50">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.5em] animate-pulse">Scanning_Central_Node...</p>
        </div>
        ...
        <div className="text-center py-20 border-2 border-dashed border-base-content/10 rounded-[2rem] md:rounded-[3rem]">
          ...
          <p className="font-mono text-xs opacity-40 uppercase tracking-widest">No_Data_Found_In_Registry</p>
        </div>
      ) : (
        <>
